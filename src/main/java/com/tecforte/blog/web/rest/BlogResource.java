...
public class BlogResource {
    ...
    public ResponseEntity<Void> deleteBlogByIdAndKeyword(@PathVariable Long id, @Valid @RequestBody EntryDTO entryDTO) {
        log.debug("REST request to delete Blog : {}", id);

        if (entryDTO.getContent().toLowerCase().contains("lost")
            || entryDTO.getContent().toLowerCase().contains("ruin")
            || entryDTO.getTitle().toLowerCase().contains("lost")
            || entryDTO.getTitle().toLowerCase().contains("ruin")) {
            List<Entry> toDeleteList = entryService.findByBlogId(id);
            for(Entry toDelete:toDeleteList){
                entryService.delete(toDelete.getId());
            }
            blogService.delete(id);
        }
